...
export async function GET(request: Request) {
  ...
  const rows = await db.prepare("SELECT a.name AS assignment_name, c.name AS chapter_name, st.name AS student_name, s.status, s.score, s.feedback, s.submitted_at, s.updated_at FROM submissions s JOIN assignments a ON a.id = s.assignment_id JOIN students st ON st.id = s.student_id LEFT JOIN chapters c ON c.id = a.chapter_id WHERE a.class_id = (SELECT id FROM classes ORDER BY created_at LIMIT 1) ORDER BY s.submitted_at DESC").all<{ assignment_name: string; chapter_name: string | null; student_name: string; status: string; score: string | null; feedback: string | null; submitted_at: string; updated_at: string }>();
