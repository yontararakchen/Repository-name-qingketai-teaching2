...
type ActivityRow = { id: string; session_id: string; activity_type: string; question_type: string; prompt: string; options: string; correct_answer: string | null; selected_student_id: string | null; selected_at: string | null; time_limit_seconds: number; allow_edit: number; deadline_at: string | null; status: string; created_at: string };
...
async function readLesson(db: NonNullable<ReturnType<typeof getDatabase>>, identity: Awaited<ReturnType<typeof getIdentity>>, classId: string) {
  ...
  const [activityResult, responseResult, discussionResult] = await Promise.all([
    db.prepare("SELECT id, session_id, activity_type, question_type, prompt, options, correct_answer, selected_student_id, selected_at, time_limit_seconds, allow_edit, deadline_at, status, created_at FROM activities WHERE session_id = ? ORDER BY created_at").bind(session.id).all<ActivityRow>(),
...
export async function POST(request: Request) {
  const body = await request.json().catch(() => null) as { action?: "start" | "restart" | "publish" | "end" | "close_activity" | "reopen_activity"; chapterId?: string; prompt?: string; options?: string[]; correctAnswer?: string; classId?: string; timeLimitSeconds?: number | string; allowEdit?: boolean; activityType?: "choice" | "poll" | "short_answer"; questionType?: "choice" | "poll" | "true_false" | "fill_blank" | "short_answer" | "quick_response" | "discussion" | "select_student" | "practical" } | null;
  ...
  const prompt = body?.prompt?.trim(); const questionType = body?.questionType ?? body?.activityType ?? "choice"; const activityType = questionType === "fill_blank" || questionType === "short_answer" || questionType === "quick_response" || questionType === "discussion" || questionType === "select_student" || questionType === "practical" ? "short_answer" : questionType === "poll" ? "poll" : "choice"; const options = (body?.options ?? []).map((option) => option.trim()).filter(Boolean).slice(0, 6); const normalizedOptions = questionType === "true_false" ? ["正确", "错误"] : options; const correctAnswer = (questionType === "choice" || questionType === "true_false") && body?.correctAnswer && normalizedOptions.includes(body.correctAnswer.trim()) ? body.correctAnswer.trim() : null; const timeLimitSeconds = Math.max(0, Math.min(3600, Number(body?.timeLimitSeconds ?? 0) || 0)); const allowEdit = body?.allowEdit === false ? 0 : 1;
  ...
  const id = newId("activity"); const createdAt = timestamp(); const deadlineAt = timeLimitSeconds > 0 ? new Date(Date.parse(createdAt) + timeLimitSeconds * 1000).toISOString() : null; await db.prepare("INSERT INTO activities (id, session_id, activity_type, question_type, prompt, options, correct_answer, time_limit_seconds, allow_edit, deadline_at, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'published', ?)").bind(id, session.id, activityType, questionType, prompt, JSON.stringify(normalizedOptions), correctAnswer, timeLimitSeconds, allowEdit, deadlineAt, createdAt).run(); await writeAudit(db, identity, "publish", "activity", id, `${prompt} · limit=${timeLimitSeconds}s · allowEdit=${allowEdit === 1}`); return NextResponse.json({ activity: { id, type: activityType, questionType, prompt, options: normalizedOptions, correctAnswer, timeLimitSeconds, allowEdit: Boolean(allowEdit), deadlineAt, status: "published" }, source: "d1" }, { status: 201 });
}
...
export async function PATCH(request: Request) {
  ...
  const activity = await db.prepare("SELECT a.session_id, a.status, a.question_type, a.correct_answer, a.allow_edit, a.deadline_at, ls.class_id FROM activities a JOIN lesson_sessions ls ON ls.id = a.session_id WHERE a.id = ? LIMIT 1").bind(body.activityId).first<{ session_id: string; class_id: string; status: string; question_type: string; correct_answer: string | null; allow_edit: number; deadline_at: string | null }>();
  ...
  if (activity.deadline_at && Date.now() >= Date.parse(activity.deadline_at)) return NextResponse.json({ error: "该活动已超过提交截止时间" }, { status: 409 });
  ...
  const existingResponse = await db.prepare("SELECT id FROM activity_responses WHERE activity_id = ? AND student_id = ? LIMIT 1").bind(body.activityId, studentId).first<{ id: string }>(); if (existingResponse && activity.allow_edit === 0) return NextResponse.json({ error: "这项活动不允许修改已提交答案" }, { status: 409 });
