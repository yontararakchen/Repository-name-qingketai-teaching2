...
export async function POST(request: Request) {
  ...
  const prompt = body?.prompt?.trim(); const questionType = body?.questionType ?? body?.activityType ?? "choice"; const activityType = questionType === "fill_blank" || questionType === "short_answer" || questionType === "quick_response" ? "short_answer" : questionType === "poll" ? "poll" : "choice"; const options = (body?.options ?? []).map((option) => option.trim()).filter(Boolean).slice(0, 6); const normalizedOptions = questionType === "true_false" ? ["正确", "错误"] : options; const correctAnswer = (questionType === "choice" || questionType === "true_false") && body?.correctAnswer && normalizedOptions.includes(body.correctAnswer.trim()) ? body.correctAnswer.trim() : null;
