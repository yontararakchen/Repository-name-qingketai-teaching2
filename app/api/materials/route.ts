...
export async function POST(request: Request) {
  ...
  const allowed = /\.(pdf|ppt|pptx|doc|docx|png|jpg|jpeg|txt|md)$/i;
  if (!allowed.test(file.name)) return NextResponse.json({ error: "仅支持 PDF、PPT、Word、图片、TXT 和 Markdown" }, { status: 415 });
  ...
  return NextResponse.json({ material: { id, chapterId, name: file.name, type: ext, size }, source: "d1", note: "当前体验环境保存文件元数据，并由材料识别演示生成待审核题目草稿。" }, { status: 201 });
}
