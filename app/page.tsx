...
export default function Home() {
  ...
  async function handleSubmit(content: string) {
    ...
    const response = await fetch("/api/submissions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ assignmentId: assignment.id, studentId: student.id, content }) });
  ...
  async function handleGrade(payload: GradePayload) {
    const response = await fetch("/api/submissions", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    if (!response.ok) throw new Error("submission_grade_failed");
    await refreshData();
    setNotice("评分和评语已保存。");
  }
  ...
  return <div className="app-shell">
    ...
    <div className="app-body">
      ...
      <main className="main-content">{loading && <div className="sync-line"><span className="sync-dot" />正在同步班级数据…</div>}{role === "teacher" ? <TeacherView page={page} onCreate={() => setShowCreate(true)} data={data} onGrade={(payload) => { void handleGrade(payload).catch(() => setNotice("评分失败，请稍后重试。")); }} /> : <StudentView page={page} data={data} submitted={submitted} onSubmit={(content) => { void handleSubmit(content).catch(() => setNotice("提交失败，请稍后重试。")); }} />}</main>
    </div>
...
function TeacherView({ page, onCreate, data, onGrade }: { page: Page; onCreate: () => void; data: ClassroomData; onGrade: (payload: GradePayload) => void }) {
  ...
  if (page === "submissions") return <TeacherSubmissions data={data} onGrade={onGrade} />;
...
function TeacherSubmissions({ data, onGrade }: { data: ClassroomData; onGrade: (payload: GradePayload) => void }) {
  const assignment = data.assignments.find((item) => item.name.includes("循环")) ?? data.assignments[0];
...
function GradeSubmissionModal({ submission, studentName, onClose, onSubmit }: { submission: Submission; studentName: string; onClose: () => void; onSubmit: (payload: GradePayload) => void }) {
  function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    onSubmit({ submissionId: submission.id, score: String(form.get("score") ?? ""), feedback: String(form.get("feedback") ?? "") });
  }
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><form className="modal-card" role="dialog" aria-modal="true" aria-labelledby="grade-title" onMouseDown={(event) => event.stopPropagation()} onSubmit={submit}><div className="modal-header"><div><p className="eyebrow">提交详情 · {studentName}</p><h2 id="grade-title">查看并评分</h2></div><button type="button" className="close-button" aria-label="关闭" onClick={onClose}>×</button></div><div className="submission-content"><p className="form-label">学生回答</p><div className="answer-preview">{submission.content || "（学生未填写文字回答）"}</div></div><label className="form-label" htmlFor="score">分数（0-100）</label><input id="score" name="score" type="number" className="form-input" inputMode="decimal" min="0" max="100" step="0.01" placeholder="例如：90" defaultValue={submission.score ?? ""} /><label className="form-label" htmlFor="feedback">教师评语</label><textarea id="feedback" name="feedback" className="answer-box small-answer" placeholder="写下对这份作业的反馈……" defaultValue={submission.feedback ?? ""} /><div className="form-actions"><button type="button" className="secondary-button" onClick={onClose}>取消</button><button type="submit" className="primary-button">保存评分</button></div></form></div>;
}
