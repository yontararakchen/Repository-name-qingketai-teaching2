...
type DraftRow = { id: string; chapter_id: string | null; student_id: string; student_name: string; chapter_name: string | null; title: string; description: string; question_ids: string; question_payload: string; status: string; created_at: string; updated_at: string; published_at: string | null; response_answers?: string | null; response_submitted_at?: string | null };
...
export async function GET(request: Request) {
  ...
  if (identity.role === "teacher") {
    const result = await db.prepare("SELECT pa.id, pa.chapter_id, pa.student_id, st.name AS student_name, c.name AS chapter_name, pa.title, pa.description, pa.question_ids, pa.question_payload, pa.status, pa.created_at, pa.updated_at, pa.published_at FROM personalized_assignments pa JOIN students st ON st.id = pa.student_id LEFT JOIN chapters c ON c.id = pa.chapter_id WHERE pa.class_id = ? ORDER BY pa.updated_at DESC").bind(context.classId).all<DraftRow>();
...
export async function POST(request: Request) {
  ...
  if (body.action === "submit") {
    ...
    const context = await getContext(db); if (!context) return NextResponse.json({ error: "暂无课程" }, { status: 404 });
    const student = identity.demo ? await db.prepare("SELECT id FROM students WHERE id = 'student_1' LIMIT 1").first<{ id: string }>() : await db.prepare("SELECT st.id FROM students st JOIN users u ON u.name = st.name WHERE u.id = ? AND st.class_id = ? LIMIT 1").bind(identity.id, context.classId).first<{ id: string }>();
...
export async function PATCH(request: Request) {
  ...
  const body = await request.json().catch(() => ({})) as { draftId?: string; status?: "published" | "rejected" };
